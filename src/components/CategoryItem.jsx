...
export default class CategoryItem extends Component {
	...
	navigateToProductDetail = async () => {
		const relatedProducts = [];
		if (this.props.item.related_ids.length !== 0) {
			Promise.all(
				this.props.item.related_ids.map((product_id) => {
	...
	changeQuantity = (val) => {
		this.setSelectedQuantity(val);
		this.setPrice(val.on_sale ? val.sale_price : val.regular_price);
	};
	...
	render() {
		...
		return (
			<TouchableWithoutFeedback
				onPress={() => this.navigateToProductDetail()}>
				<View style={parentContainerStyle}>
					<View style={[styles.container, containerStyle]}>
						...
						{item.sale_price !== '' && profile.role !== 'LFB Role' && (
							<View
								style={[
